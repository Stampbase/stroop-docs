...
- This repository is the canonical public specification. If the documentation
  and an implementation disagree, that is a bug in one of them — say so in an
  issue rather than quietly changing the spec to match the code.
- Keep protocol invariants in `identity-model.md` and treat them as
  architecture constraints, not suggestions.
- Do not document funding strategy, budgets, grant applications, or internal
  business planning here.
- Do not publish exploitable detail about an unresolved vulnerability.
- Use synthetic addresses and identifiers in every example.
